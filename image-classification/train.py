...
train_dataset = LoadDataset(image_size=args.img_size, image_depth=args.img_depth, root=args.root,
                            transform=transforms.Compose([transforms.RandomAffine(40, scale=(.85, 1.15), shear=0, resample=0),
                                transforms.RandomHorizontalFlip(),
                                transforms.RandomPerspective(distortion_scale=0.2),
                                transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5),
                                 transforms.ToTensor()]))
test_dataset = LoadDataset(image_size=args.img_size, image_depth=args.img_depth, root=args.root, transform=transforms.ToTensor())
