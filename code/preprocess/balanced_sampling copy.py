...
dataset_keys = [
    ...
    "imagewidth", "imageheight", "gsd", "latitude", "longitude", "location",
...
def validate(df, ratio):
    ...
    for label in labels:
        ...
        model.Add(df[label].sum(axis = 1).dot(row_selection) <= label_sample)
...
validation_images = validation_images.loc[:, (validation_images != 0).any(axis=0)]
validation_images = pd.DataFrame(data = validation_images.columns.unique())

# Convert validation back into patch data
validation = (
...
n_validation = len(validation.drop_duplicates(['imagepath', 'patchcorners']))
...
print("Validation patches: ", n_validation)
