...
def transforms(instance, mask, gsd, target_gsd):
    min_transform = 0.75
    max_transform = 1.25
    # quality
        ...
        for image_path, patches in dataset:
            ...
            for patch_points, instances in patches:
                ...
                if instances.iloc[0]["instance_type"] == "artificial":
                    ...
                    bird_sizes = [sum(bird.size)/2 for bird in instances["instance_object"]]
                    bird_size = sum(bird_sizes)/len(bird_sizes)
                    spacing = range(int(-bird_size * 0.25), int(target_patchsize - bird_size * 0.75), int(bird_size * 0.5))
...
if len(file_path_variable) > 0:
    ...
    if check =="yes":
        ...
        dataset_keys = [
            ...
            "instance_overlap", "instance_area", "instance_pose"]
                    ...
                    for height_index in range(n_crops_height):
                        for width_index in range(n_crops_width):
                            ...
                            if instance_in_patch == False:
                                ...
                                data["instance_pose"].append("null")
                                ...
                                data["instance_area"].append("null")
        ...
        print("Test Instances:\n{}\n".format(test_instances.query("instance_shape_type == 'polygon'").query("instance_overlap >= {0}".format(min_overlap)).query("instance_class.isin({0})".format(included_classes), engine="python").instance_class.value_counts()))
        ...
        train_instances = (
            ...
            .drop('_merge', axis=1)
            .reset_index(drop = True))
        print("Train Instances:\n{}\n".format(train_instances.query("instance_shape_type == 'polygon'").query("instance_overlap >= {0}".format(min_overlap)).query("instance_class.isin({0})".format(included_classes), engine="python").instance_class.value_counts()))
        ...
        for image_path, patches in train_backgrounds.groupby("image_path"):
            ...
            for patch_points, _ in patches.groupby("patch_points"):
                ...
                for index, row in instances.iterrows():
                    instance_object, instance_mask = transforms(row["instance_object"], row["instance_crop"], row["image_gsd"], target_gsd)
                    train_row = pd.DataFrame([[image_path, image_gsd, patch_points, instance_object, row["instance_class"], instance_mask, "artificial", 1.0, row["instance_area"], "polygon"]], columns = ["image_path", "image_gsd", "patch_points", "instance_object", "instance_class", "instance_mask", "instance_type", "instance_overlap", "instance_area", "instance_shape_type"])
